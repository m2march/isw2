...
importantes.
	...
	{Accepted} % estado (completada, bloqueada, etc.)
	\task
		{TA142} % ID en el Rally
		{Implementar la funcionalidad deseada} % titulo
		{
	...
	\task
		...
		{Completed} % estado (completada, bloqueada, etc.)
\userStory
	{US104} % ID en el Rally
	{Como desarrollador quiero filtrar las ofertas según el rango de precios del usuario} % titulo
...
la US124
	...
	\task
		{TA57} % ID en el Rally
		{Testear la funcionalidad realizada según los criterios de aceptación} % titulo
		{
...
a través de su interfaz.
	...
	\task
		{TA62} % ID en el Rally
		{Validar el funcionamiento.} % titulo
...
total de la aplicación a, por ejemplo, Capital Federal.
	...
	{Defined} % estado (completada, bloqueada, etc.)
\userStory
	{US111} % ID en el Rally
	{Como desarrollador quiero filtrar las ofertas por zona geográfica} % titulo
	{
...
el sistema o pueden ser delimitadas por el usuario durante la consulta.
	...
	\task
		{TA44} % ID en el Rally
		{Investigar como vamos a representar una Zona y los posibles recursos que vamos a utilizar para determinarlas.} % titulo
		{
...
\userStory
	{US83} % ID en el Rally
	{Como usuario quiero realizar búsquedas filtrando por la estrategia "gastando lo menos posible"} % titulo
	{
...
\userStory
	{US89} % ID en el Rally
	{Como desarrollador espero poder agregar, cambiar y eliminar productos de la lista de productos soportados junto con su respectiva unidad} % titulo
	{
...
* Un usuario quiere valorar la experiencia de compra de un producto que consiguió a través de una oferta del sistema. El usuario ingresará su valoración, y en consultas subsiguientes, los usuarios la verán cuando reciban la oferta.
	...
	{Defined} % estado (completada, bloqueada, etc.)
\userStory
	{US120} % ID en el Rally
	{Como usuario de la comunidad de Twitter me gustaría que mis tweets se prioricen a la hora de mostrar las ofertas según su cantidad de favs o retweets} % titulo
...
* En una consulta cuyo resultado incluya a este tweet, el sistema deberá darle prioridad por sobre los otros tweets que tengan menor cantidad de favs y retweets.
	...
	{Defined} % estado (completada, bloqueada, etc.)
\userStory
	{US121} % ID en el Rally
	{(REPETIDO???)Como usuario quiero elegir mas de un producto para obtener las mejores ofertas de todos ellos y visualizarlas en un mapa} % titulo
	{
