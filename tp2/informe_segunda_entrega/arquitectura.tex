...
\begin{itemize}
		...
		\NonStandardConector{Client-Server Async}{Funciona igual que un client-server, pero el cliente no se cuelga esperando una respuesta, y el server puede retornar su respuesta de manera asincrónica. Lo utilizamos para poder aprovechar el dispatch que el conector tiene sobre las instancias de servidores que hay disponibles que un conector async no soporta.}
		\NonStandardConector{Secure Client-Server}{Este conector resulta muy similar a Cliente-Servidor. Se diferencia únicamente en que antes de que el cliente envíe una consulta al servidor, este \emph{encripta} el mensaje, y luego se lo envía. Ya en el servidor, este desencripta el mensaje y lo interpreta. Para la respuesta, de forma similar a la mencionada anteriormente, ahora es el servidor quien encripta el mensaje y el cliente quien lo desencripta.}
\end{itemize}
...
Los \textsf{Web Mining Robots} son un conjunto de componentes que cada una está programa para revisar un sitios web específicos con el fin de encontrar nuevas ofertas, para luego enviársela al \textsf{Comunicador de Robots}. Esto otorga una gran flexibilidad para poder fácilmente eliminar/modificar/agregar nuevos robots debido a que son completamente independientes entre ellos (esto posibilita cumplir con el atributo de \emph{Modificabilidad} numero 13).En caso de ser incapaces de entender la estructura del sitio que relevan, debido a que el mismo cambio recientemente (o si detectan grandes cambios y podría haber peligro de estar entendiendo mal la ofertas), los Robots se detienen y mandan una alerta avisando lo sucedido al \textsf{Comunicador de Robots}, este último le avisa al \textsf{Subsistema de Alertas} para darle aviso a los Admins (esto corresponde al atributo número 14 de \emph{detección y prevención de fallas}).
\vspace{9pt}


\subsection{Subsistemas de Información de Negocio}

En el diagrama del \textsf{Subsistema de Negocio de Ofertas} (figura \ref{arch:neg_ofertas}) se encuentran varios subsistemas que manejan información de negocio. Estos subsistemas son: el \textsf{Subsistema de Publicidades}, el \textsf{Subsistema de Rubros y Productos}, el \textsf{Subsistema de Reglas}, el \textsf{Subsistema Pref. de Usuario}, el \textsf{Subsistema de Reputación}, el \textsf{Subsistema de Filtros y Estrategias}, el \textsf{Subsistema de Manejo de Spam} y el \textsf{Subsistema de Alertas}.
En todos los casos estos subsistemas son accedidos mediante una conexión de tipo \emph{cliente-servidor} ya que proveen un servicio respecto a la información de negocio contenida en cada uno. Además, se busca permitir concurrencia en el acceso a cada uno de los subsistemas.
...
\nota{Juansis va a escribir y hacer dibujitos aca. March va a ayudar también.}
