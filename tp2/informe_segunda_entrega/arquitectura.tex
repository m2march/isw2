...
El manejo de pedidos de ofertas provenientes de clientes se resuelve de la siguiente manera, el mismo es aceptado por un \textsf{Manejador de pedidos de API} o \textsf{Manejador de pedidos de aplicación} este recibe el pedido y le asigna un identificador de pedido. Luego arma un mensaje con su identificador, el identificador del pedido y el contenido del pedido y env\'ia dicho mensaje por medio de nuestro conector custom hacia alg\'un \textsf{Query Dispatcher}. Cabe destacar que el manejador tiene memoria de aquellos pedidos que todavia le debe responder a sus clientes. El \textsf{Query Dispatcher} toma el mensaje y lo pone en la cola de trabajo del \textsf{Subsistema de Query}. Al terminar el procesamiento del mensaje el resultado del mismo es devuelto en la cola de \textsf{QueryDispatcher} que seg\'un el identificador del mensaje original lo envia asincrónicamente hacia el manejador que lo creo. El manejador recibe el resultado y se lo devuelve al cliente que origino el pedido.     

 \vspace{9pt}

Esta forma de manejar los pedidos de los clientes sobre ofertas satisface el atributo de calidad 3.8 que se refiere a \emph{disponibilidad} porque ning\'un pedido deberia ser rechazado asumiendo una coherente asignacion de instancias de manejadores, dispatchers y de elementos procesadores de queries. El volumen esperado de este tipo de interacción es muy grande.

\subsubsection{Manejo de mensajes de reputación y preferencias}

El manejo de mensajes de reputaci\'on y preferencias se hace de manera diferente al de pedido de ofertas porque el volumen esperado de los mismos es menor y además su resoluci\'on es muchisimo mas r\'apida. Usamos entonces un conector client-server convencional entre el \textsf{Manejador} y los sistemas de reputación y preferencias.
